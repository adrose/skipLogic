...
for(i in 1:dim(all.folds)[1]){
    ...
    results <- foreach(z=seq(1,nsim), .combine=rbind) %dopar% {
        ...
        try(IRT_scores_no_SL <- score.irt(modp,dat$items)$theta1)             # calculate IRT scores before applying skip logic
        ...
        try(IRT_scores <- score.irt(mod,dat$items)$theta1)                    # calculate IRT scores after applying skip logic
